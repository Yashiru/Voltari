...
A **ratchet**, measured rather than chosen: the recorded baseline is **81.6%**
across **every mutation site in the purity-enforced layers** — 407 of 499, not a
sample — and it may not go down. See decision 0021 for why the target is a
measurement rather than a number.

> Measured at 402 of 499 on 2026-09-10, plus five mutants killed immediately
> afterwards in `battle_view.gd` and re-verified by an exhaustive pass over that
> file, which went from 75% to 92.5%. The population did not change between the
> two: only tests were added.
>
> **It goes stale quickly and the percentage hides it.** The figure moved 80.3%
> to 81.6% across a whole block of work, while seven survivors sat in newly
> written code — a number that looked flat over a real gap. Read the survivor
> list.
>
> Two of the five needed the *exact* boundary: `<` and `<=` disagree only there,
> so a test that aimed well past the end passed either way and proved nothing.
...
**Most of the remaining percentage is not work left.** Of 92 survivors, 88 are
ones no passing test can kill, in three kinds:

| Kind | Count | Why it cannot be killed |
|------|-------|-------------------------|
| Asserts, and the `return` behind them | 63 | Abstract base methods and preconditions. The original aborts on the very input that would tell the mutant apart, so chasing these means testing that an abstract method is abstract. |
| Guarded or unreachable comparisons | 15 | A `>` or `<` immediately under an `if a != b`, where the operands are never equal; a `?:` fallback for a null nothing produces; the tail after an `assert(false)`. |
| Field defaults | 10 | Every constructor overwrites them before anything reads. |
...
**The four that are real**, named so they are worked on rather than rediscovered:

| Site | What no test pins |
|------|-------------------|
| `capture.gd:94` | `_floor_sqrt` at 2 — the Newton loop's own boundary |
| `capture.gd:98` | the Newton step itself; may be an equivalent mutant |
| `post_battle.gd:272` | a participant marked as fainted |
| `turn_engine.gd:349` | a `return true` nothing distinguishes |

`battle_view.gd:26` used to sit here and is closed: a sentinel of `-1` is safe
and one of `+1` collides with a real health proportion, and no test comparing
against the constant could tell them apart, because the constant moves too. The
test that kills it asserts the sentinel is outside the range rather than equal to
itself.
